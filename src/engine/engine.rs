...
pub fn search_for_best_move(board: &Chessboard) {
    ...
    if let (Some(best_move), calculations) = calculate(board, &mut checked_positions, max_depth, 1)
    {
        println!(
...
fn get_valid_moves_in_position(board: &Chessboard) -> (Vec<PossibleMove>, bool){
