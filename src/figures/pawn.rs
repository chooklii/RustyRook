...
impl Pawn {
    fn check_taking(&self, board: &Chessboard, position: usize) -> Option<usize> {
        if board.positions.get(position) {
            ...
            let field_color = field.unwrap().as_ref().get_color();
...
impl Figure for Pawn {
    ...
    fn possible_moves(&self, board: &Chessboard, own_position: &usize) -> Vec<usize> {
        ...
        let one_step_forward = self.calculate_next_position(own_position, 8);
        ...
        if !board.positions.get(one_step_forward) {
            ...
            // two fields forward
            let two_steps_forward = self.calculate_next_position(own_position, 16);
...
mod tests {
    ...
    fn test_take_from_a_to_h() {
        let mut positions = Bitmap::<64>::new();
        let mut figures: HashMap<usize, Box<dyn Figure>> = HashMap::new();
